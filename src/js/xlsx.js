...
function make_xlsx_lib(XLSX) {
    ...
    var make_ssf = function make_ssf(SSF) {
        ...
        function eval_fmt(fmt, v, opts, flen) {
            ...
            if (nstr.length > 0) {
                ...
                for (i = 0; i < out.length; ++i)
                    if (out[i] != null && out[i].t != 't' && out[i].v.indexOf(".") > -1) { decpt = i; break; }
    ...
    var CFB = (function _CFB() {
        ...
        */
        ...
        function warn_or_throw(wrn, msg) {
            if (wrn) { if (typeof console !== 'undefined') console.error(msg); } else throw new Error(msg);
        }
    ...
    function WriteShift(t, val, f) {
        ...
        if (f === 'dbcs') {
            ...
            case 8:
                size = 8;
                if (f === 'f') { write_double_le(this, val, this.l); break; }
                /* falls through */
            case 16:
                break;
            case -4:
    ...
    var make_offcrypto = function(O, _crypto) {
        ...
        if (typeof _crypto !== 'undefined') crypto = _crypto;
            ...
            try { crypto = undefined; } catch (e) { crypto = null; }
        }
    ...
    var CT_LIST = (function() {
        ...
        keys(o).forEach(function(k) {
            ["xlsm", "xlam"].forEach(function(v) { if (!o[k][v]) o[k][v] = o[k].xlsx; });
        });
    ...
    function load_props_pairs(HP, TOP, props, opts) {
        ...
        if (typeof HP == "string") v = parseVector(HP, opts);
            ...
            for (var j = 0; j < HP.length; ++j) v = v.concat(HP[j].map(function(hp) { return { v: hp }; }));
    ...
    function xlml_write_docprops(Props, opts) {
        ...
        keys(XLMLDocPropsMap).map(function(m) {
            ...
            else if (m === true || m === false) { m = m ? "1" : "0"; } else if (m instanceof Date) m = new Date(m).toISOString().replace(/\.\d*Z/, "");
    ...
    var DBF = (function() {
                ...
                for (C = 0; C != fields.length; ++C) {
                    ...
                    switch (fields[C].type) {
                        ...
                        case 'O':
                            ...
                            break;
                        case 'B':
                            if (vfp && fields[C].len == 8) { out[R][C] = dd.read_shift(8, 'f'); break; }
                            /* falls through */
                        case 'G':
                        case 'P':
                            dd.l += fields[C].len;
        ...
        function dbf_to_workbook(buf, opts) {
            try { return sheet_to_workbook(dbf_to_sheet(buf, opts), opts); } catch (e) { if (opts && opts.WTF) throw e; }
    ...
    var PRN = (function() {
        ...
        function dsv_to_sheet_str(str, opts) {
            ...
            function finish_cell() {
                ...
                if (cc == sepcc) ++C;
                else { C = 0;++R; if (o.sheetRows && o.sheetRows <= R) return true; }
            }
        ...
        xml.replace(tagregex, function xml_agile(x) {
            ...
            switch (strip_ns(y[0])) {
                ...
                case '</encryption>':
                    break;
                case '<keyData':
                    KeyData.forEach(function(k) { o[k] = y[k]; });
                    break;
                case '<dataIntegrity':
                ...
                case '<xf/>':
                    ...
                    if (xf.numFmtId > 0x188) {
                        for (i = 0x188; i > 0x3c; --i)
                            if (styles.NumberFmt[xf.numFmtId] == styles.NumberFmt[i]) { xf.numFmtId = i; break; }
                    }
    ...
    function parse_Theme(blob, length, opts) {
        ...
        blob.l = end;
        var zip;
        try { zip = new jszip(data); } catch (e) { return; }
    ...
    function parse_cc_bin(data, name, opts) {
        ...
        recordhopper(data, function hopper_cc(val, R_n, RT) {
            switch (RT) {
                ...
                default:
                    if ((R_n || "").indexOf("Begin") > 0) { /* empty */ } else if ((R_n || "").indexOf("End") > 0) { /* empty */ } else if (!pass || opts.WTF) throw new Error("Unexpected record " + RT + " " + R_n);
            }
        });
        return out;
    }
	...
	*/
    ...
    function write_WSVIEWS2(ba, ws, Workbook) {
        write_record(ba, "BrtBeginWsViews"); { /* 1*WSVIEW2 */
            /* [ACUID] */
    ...
    function write_ws_bin(idx, opts, wb, rels) {
        ...
        var s = wb.SheetNames[idx],
            ws = wb.Sheets[s] || {};
        var c = s;
        try { if (wb && wb.Workbook) c = wb.Workbook.Sheets[idx].CodeName || c; } catch (e) {}
    ...
    function parse_xlml_xml(d, _opts) {
        ...
        while ((Rn = xlmlregex.exec(str))) switch (Rn[3]) {
            ...
            case 'Worksheet':
                ...
                break;
            case 'Table':
                if (Rn[1] === '/') { if ((tmp = state.pop())[0] !== Rn[3]) throw new Error("Bad state: " + tmp.join("|")); } else if (Rn[0].slice(-2) == "/>") break;
                else {
            ...
            case 'NumberFormat':
                ...
                if (ssfidx == 0x188)
                    for (ssfidx = 0x39; ssfidx != 0x188; ++ssfidx)
                        if (SSF._table[ssfidx] == null) { SSF.load(stag.nf, ssfidx); break; }
                break;
            ...
            case 'Styles':
            case 'Workbook':
                if (Rn[1] === '/') { if ((tmp = state.pop())[0] !== Rn[3]) throw new Error("Bad state: " + tmp.join("|")); } else state.push([Rn[3], false]);
                break;
            ...
            case 'AutoFilter':
                if (Rn[1] === '/') { if ((tmp = state.pop())[0] !== Rn[3]) throw new Error("Bad state: " + tmp.join("|")); } else if (Rn[0].charAt(Rn[0].length - 2) !== '/') {
            ...
            case 'WorksheetOptions':
                if (Rn[1] === '/') { if ((tmp = state.pop())[0] !== Rn[3]) throw new Error("Bad state: " + tmp.join("|")); } else if (Rn[0].charAt(Rn[0].length - 2) !== '/') state.push([Rn[3], true]);
                break;
    ...
    */
    ...
    function safe_format_xf(p, opts, date1904) {
        ...
        if (!opts || opts.cellText !== false) try {
            if (p.t === 'e') { p.w = p.w || BErr[p.v]; } else if (fmtid === 0 || fmtid == "General") {
                if (p.t === 'n') {
                    if ((p.v | 0) === p.v) p.w = SSF._general_int(p.v);
                    else p.w = SSF._general_num(p.v);
    ...
    var HTML_ = (function() {
        function html_to_sheet(str, _opts) {
            ...
            for (i = 0; i < rows.length; ++i) {
                ...
                if (hd == "<tr") {
                    ++R;
                    if (opts.sheetRows && opts.sheetRows <= R) {--R; break; }
                    C = 0;
                    continue;
                }
                ...
                for (j = 0; j < cells.length; ++j) {
                    ...
                    var o = { t: 's', v: m };
                    if (opts.raw || !m.trim().length || _t == 's') {} else if (m === 'TRUE') o = { t: 'b', v: true };
        ...
        for (; _R < rows.length && R < sheetRows; ++_R) {
            ...
            for (_C = C = 0; _C < elts.length; ++_C) {
                ...
                if (v != null) {
                    ...
                    else if (opts.raw || v.trim().length == 0 || _t == "s") {} else if (v === 'TRUE') o = { t: 'b', v: true };
    ...
    var parse_content_xml = (function() {
        ...
        return function pcx(d, _opts) {
            ...
            while ((Rn = xlmlregex.exec(str))) switch ((Rn[3] = Rn[3].replace(/_.*$/, ""))) {
                ...
                case 'chart': // TODO
                    if (Rn[1] === '/') { if ((tmp = state.pop())[0] !== Rn[3]) throw "Bad state: " + tmp; } else if (Rn[0].charAt(Rn[0].length - 2) !== '/') state.push([Rn[3], false]);
                    textp = "";
                    textpidx = 0;
                    break;
    ...
    function sheet_add_json(_ws, js, opts) {
        ...
        if (offset)
            for (C = 0; C < hdr.length; ++C) ws[encode_col(C + _C) + __R] = { t: 's', v: hdr[C] };
        ws['!ref'] = encode_range(range);
        return ws;
    }
